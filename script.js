...
if (contactForm) {
    ...
    const showContactStatus = (message, isError = false) => {
        ...
        contactStatus.style.color = isError ? '#ff8a8a' : '#7ef0b9';
    };
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        if (!name || !email || !message) {
            showContactStatus('Silakan isi nama, email, dan pesan terlebih dahulu.', true);
        ...
        try {
            showContactStatus('Membuka aplikasi email Anda...');

            const popup = window.open(gmailLink, '_blank');
            if (!popup) {
                window.location.href = mailtoLink;
            }
        } catch (error) {
            showContactStatus('Terjadi masalah saat membuka email. Silakan coba lagi.', true);
        } finally {
            contactForm.reset();
            btn.innerHTML = originalText;
            btn.disabled = false;
